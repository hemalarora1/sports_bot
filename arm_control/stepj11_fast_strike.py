#!/usr/bin/env python3
"""
Step J11: J9 fast strike — velocity saturation disabled, FR3 position-dependent capping.

What's new vs J9:
  - --no-vel-saturation: at startup writes high saturation limits to Redis so
    the OpenSai joint_task software clamp is effectively off.
  - vel_cap comes from the FR3 driver's position-dependent soft-zone limits at
    q_strike (not the static XML saturation values). This keeps us within what
    the hardware firmware will actually accept.
  - Per-step FR3 clamp during execution: if a Hermite qdot_goal exceeds the
    soft limit at that q_goal, it is clipped before being written to Redis.
    This prevents hardware reflex aborts on the approach phase.
  - Commit log prints per-joint hard/soft ratio so you can immediately answer
    "which joint is being violated?".

Everything else (aiming logic, home recovery, tracker, IK) is identical to J9.
"""
...
# Tracker defaults tuned 2026-06-02
J9_TRACKER_GRAVITY_MPS2 = 11.0
J9_TRACKER_DRAG_COEFFICIENT = 0.10
J9_TRACKER_SIMULATION_DT_S = 0.001
J9_TRACKER_MIN_HISTORY = 4

# Redis keys for velocity saturation (written at startup when --no-vel-saturation)
_VEL_SAT_KEY = f"{NS}::{JOINT_CTRL}::joint_task::velocity_saturation_limit"
# Large-but-finite value (rad/s) written to effectively disable software saturation.
# The FR3 hardware cap is ~3 rad/s per joint; 4.0 leaves OpenSai out of the loop.
_VEL_SAT_DISABLED_RAD = np.full(7, 4.0)


# ---------------------------------------------------------------------------
# FR3 position-dependent velocity limits (mirrored from the driver)
# ---------------------------------------------------------------------------
...
def _fr3_driver_velocity_bands_rad(q: np.ndarray) -> dict:
    ...
    hard = 0.6  # hard zone width (rad/s) — mirrors FrankaPanda driver
    soft = 0.9  # soft zone width (rad/s)
    return {
        "raw_min": raw_min,
...
def _fr3_symmetric_soft_cap(q: np.ndarray) -> np.ndarray:
    """Symmetric (|qdot|) soft-zone velocity cap at configuration q."""
    bands = _fr3_driver_velocity_bands_rad(q)
    return np.maximum(np.minimum(bands["soft_max"], -bands["soft_min"]), 0.0)


def _fr3_clamp_qdot(q: np.ndarray, qdot: np.ndarray) -> np.ndarray:
    """Clamp qdot to the FR3 soft zone at configuration q."""
    bands = _fr3_driver_velocity_bands_rad(q)
    return np.clip(qdot, bands["soft_min"], bands["soft_max"])
...
def _trajectory_diagnostics(chain, q_start: np.ndarray, cand: ImpactCandidate, offset_link7: np.ndarray) -> dict:
    ...
    worst_hard: dict = {"ratio": 0.0}
    worst_soft: dict = {"ratio": 0.0}
    phases = (
...
def _read_xml_velocity_limits(r: redis.Redis) -> np.ndarray:
    ...
    raw = r.get(key)
    if raw is None:
        print(f"[J11] WARNING: velocity limits key not found ({key}); using hardcoded defaults")
        return limits
    try:
        parsed = np.asarray(json.loads(raw), dtype=float)
        if parsed.shape == (7,):
            print(f"[J11] velocity limits from OpenSai: {np.degrees(parsed).round(1).tolist()} deg/s")
            return parsed
    except Exception:
        pass
    print("[J11] WARNING: could not parse velocity limits from Redis; using hardcoded defaults")
    return limits


def _disable_vel_saturation(r: redis.Redis) -> None:
    """Write a high saturation limit to Redis so the OpenSai software clamp is inactive."""
    r.set(_VEL_SAT_KEY, json.dumps(_VEL_SAT_DISABLED_RAD.tolist()))
    print(
        f"[J11] --no-vel-saturation: wrote {np.degrees(_VEL_SAT_DISABLED_RAD).round(1).tolist()} deg/s "
        f"to {_VEL_SAT_KEY}"
    )
...
def _make_impact_candidate(
    *,
    ...
    vel_cap: np.ndarray,           # swing / home-recovery cap (from xml or FR3)
    impact_vel_frac: float,
    fr3_vel_frac: float,           # additional safety margin on FR3 soft-zone cap
    reach_m: float,
    z_min: float,
    z_max: float,
    world_z_min_m: float,
    ...
    # Cap: tighter of (xml_vel_cap * impact_vel_frac) and (FR3 soft zone * fr3_vel_frac)
    fr3_soft = _fr3_symmetric_soft_cap(q_strike)
    qdot_cap = np.maximum(
        np.minimum(
            vel_cap * max(0.05, impact_vel_frac),
            fr3_soft * max(0.05, fr3_vel_frac),
        ),
        1e-6,
    )
...
def _run_impact_strike(
    ...
    *,
    ...
    dq_start = get_vec(r, SENSOR_JOINT_VELS, 7)
    if q_start is None:
        q_start = cand.q_strike.copy()
    # Use actual measured velocity as the spline's initial condition so the
    # commanded trajectory is smooth at t=0 (no velocity step = no jerk spike).
    if dq_start is None:
        dq_start = np.zeros(7)
    ...
    # Actual velocity captured at the approach→decel junction for smooth handoff.
    dq_at_junction: np.ndarray = cand.qdot_strike.copy()
    junction_sampled = False
    ...
    tr("impact_start", candidate=_candidate_snapshot(cand), diagnostic=_driver_diag_snapshot(r),
       dq_start_deg_s=np.degrees(dq_start))
    print(
        f"[J11 impact] start: strike_s={strike_s:.3f}s decel_s={decel_s:.3f}s "
        f"v_cmd_W=[{cand.achieved_v_W[0]:+.2f},{cand.achieved_v_W[1]:+.2f},{cand.achieved_v_W[2]:+.2f}] m/s"
        + (" (FR3 clamp ON)" if clamp_fr3 else "")
    )
    if full_segment_logs:
        print("[J11 impact] q_start:    " + _fmt_q(q_start))
        print("[J11 impact] dq_start:   " + _fmt_q(dq_start))
        print("[J11 impact] q_strike:   " + _fmt_q(cand.q_strike))
        print("[J11 impact] qdot_hit:   " + _fmt_q(cand.qdot_strike))
        print("[J11 impact] q_stop:     " + _fmt_q(cand.q_stop))

    while True:
        now     = time.perf_counter()
        elapsed = now - t0

        # At the approach→decel transition, snapshot the actual joint velocity
        # so the decel Hermite starts from where the arm actually is rather than
        # from the planned qdot_strike (which may not have been reached exactly).
        if not junction_sampled and elapsed >= strike_s:
            dq_junction = get_vec(r, SENSOR_JOINT_VELS, 7)
            if dq_junction is not None:
                dq_at_junction = dq_junction
            junction_sampled = True

        if elapsed <= strike_s:
            q_goal, qdot_goal = _hermite(q_start, dq_start, cand.q_strike, cand.qdot_strike, elapsed, strike_s)
        else:
            q_goal, qdot_goal = _hermite(cand.q_strike, dq_at_junction, cand.q_stop, zeros, elapsed - strike_s, decel_s)

        if clamp_fr3:
            qdot_clamped = _fr3_clamp_qdot(q_goal, qdot_goal)
            if not np.allclose(qdot_clamped, qdot_goal, atol=1e-4):
                n_clamped += 1
            qdot_goal = qdot_clamped
        ...
        if now >= next_sample:
            ...
            if dq is not None:
                ...
                dt_s = max(1e-6, now - prev_t)
                qdot_peak = np.maximum(qdot_peak, np.abs(np.degrees((q - prev_q) / dt_s)))
    ...
    print(
        f"[J11 segment] impact: {status} strike_err={strike_err_deg:.2f}deg "
...
def run_loop(
    *,
    ...
    print(f"[J11] running at {rate_hz:.0f} Hz -- fast impact striker (FR3-aware vel cap)")
    print(
        f"[J11] strike_plane_x={strike_plane_x_world:+.3f}m commit_tti={commit_tti:.3f}s "
        f"guard={timing_guard_s:.3f}s try_late_slack={try_late_slack_s:.3f}s"
    )
    print(
        f"[J11] impact velocity target W: forward=+{forward_speed_mps:.2f} m/s, "
        f"up=+{up_speed_mps:.2f} m/s, decel_s={decel_s:.2f}s"
    )
    print(
        f"[J11] vel cap: swing_vel_frac={swing_vel_frac:.2f} impact_vel_frac={impact_vel_frac:.2f} "
        f"fr3_vel_frac={fr3_vel_frac:.2f} (tighter of xml*impact and fr3_soft*fr3) "
        f"clamp_fr3={'ON' if clamp_fr3 else 'OFF'}"
    )
    print()
    ...
    while True:
        ...
        tr("candidate", tick=tick_i, reason=cand_reason, candidate=_candidate_snapshot(cand),
           intercept=_intercept_snapshot(intercept))
        if cand is None:
        ...
        if loop_t - last_print_t >= max(0.05, log_period_s):
            ...
            print(
                f"[J11] {state:6s} tti={aged_tti:+.3f}s strike_W={_fmt_v3(cand.strike_W)} "
            ...
            if verbose_tracking:
                ...
                print(
                    f"[J11]   q_to_strike={q_delta:.1f}deg@q{q_joint} "
        ...
        if commit_now:
            if late_try and not feasible:
                ...
                traj_diag = _trajectory_diagnostics(chain, q_cur, cand, offset_link7)
                print(
                    f"[J11] TRY_STRETCH: ball_time={old_strike_s:.3f}s < Tmin={cand.min_strike_s:.3f}s; "
                    f"swinging safely over {cand.strike_time_s:.3f}s"
                )

            hard = traj_diag["fr3_worst_hard"]
            soft = traj_diag["fr3_worst_soft"]
            fr3_at_s = traj_diag.get("fr3_at_strike", {})
            # Per-joint velocity headroom at strike config — this answers William's question
            fr3_soft_cap = _fr3_symmetric_soft_cap(cand.q_strike)
            qdot_abs = np.abs(cand.qdot_strike)
            headroom_pct = 100.0 * (1.0 - qdot_abs / np.maximum(fr3_soft_cap, 1e-6))
            ...
            print("\n" + "=" * 70)
            print(
                f"[J11] COMMIT throw {active_throw_id}: tti={aged_tti:.3f}s "
            ...
            print(
                f"[J11]   strike_W={_fmt_v3(cand.strike_W)} strike_A={_fmt_v3(cand.strike_A)} "
                f"v_W desired={_fmt_v3(cand.desired_v_W)} achieved={_fmt_v3(cand.achieved_v_W)}"
            )
            print(
                f"[J11]   FR3 worst_hard={100*hard['ratio']:.0f}%@q{hard['joint']} "
                f"worst_soft={100*soft['ratio']:.0f}%@q{soft['joint']} (across full trajectory)"
            )
            # Per-joint headroom at the strike pose — easy to read off which joint is tight
            headroom_str = "  ".join(
                f"q{i+1}:{hp:+.0f}%" for i, hp in enumerate(headroom_pct)
            )
            print(f"[J11]   FR3 soft headroom at strike:  {headroom_str}")
            print("=" * 70 + "\n")
            ...
            if _safety_tripped(r):
                print("[J11] safety torque after impact; recovering slowly")
            ...
            print(
                ...
                f"home_ok={home_ok} fr3_clamped={result['n_fr3_clamped_steps']}] "
                f"tracker_rejects={_fmt_counts(reject_counts)}"
            )
            tr("commit_done", tick=tick_i, throw_id=active_throw_id, outcome=outcome,
               result=result, home_ok=home_ok, reject_counts=reject_counts)
            reject_counts = {}
            active_cand = None
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="J11: fast impact striker with FR3-aware velocity capping.",
    ...
    ap.add_argument("--forward-speed-mps", type=float, default=1.50,
                    help="Target paddle forward speed at impact (m/s). Higher than J9 default.")
    ap.add_argument("--up-speed-mps", type=float, default=0.15)
    ap.add_argument("--impact-vel-frac", type=float, default=0.90,
                    help="Fraction of xml_vel_limits to allow at impact (first of two caps).")
    ap.add_argument("--fr3-vel-frac", type=float, default=0.85,
                    help="Fraction of FR3 soft-zone limit at q_strike to allow (second cap, "
                         "prevents hardware reflex aborts).")
    ap.add_argument("--no-vel-saturation", action="store_true",
                    help="Write a high velocity saturation limit to Redis at startup, "
                         "effectively disabling the OpenSai software clamp.")
    ap.add_argument("--no-fr3-clamp", action="store_false", dest="clamp_fr3",
                    help="Disable per-step FR3 soft-zone clamping during strike execution "
                         "(not recommended; use only for diagnostics).")
    ap.set_defaults(clamp_fr3=True)
    ...
    if args.no_vel_saturation:
        _disable_vel_saturation(r)
    ...
    if q_start_check is None or (not args.allow_zero_joint_start and np.max(np.abs(q_start_check)) < 1e-6):
        sys.exit(
            "[J11] refusing to start: sensed joint state is missing or exactly zero. "
    ...
    if args.home_joints_deg is not None:
        ...
        print(f"[J11] custom home pose: {_fmt_q(q_ready_seed)} (delta {q_delta:.1f}deg@q{q_joint})")
    ...
    if not args.no_fixed_paddle_ori:
        ...
        print(
            f"[J11]   paddle_open={args.paddle_open_deg:+.1f}deg "
    ...
    if args.ready_pose == "strike-center":
        ...
        if _joints_ok(q_candidate) and _ik_ok(ready_err, ready_ori, args.ik_tol_mm / 1000.0, args.max_ori_err_deg, R_A_link7_home):
            ...
            print(f"[J11] WARNING: strike-center IK failed; using home pose")
            ready_arm_A = None
    ...
    try:
        run_loop(
            ...
            fr3_vel_frac=args.fr3_vel_frac,
            clamp_fr3=args.clamp_fr3,
