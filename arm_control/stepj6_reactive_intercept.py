...
def _return_home_blocking(
    ...
    *,
    ...
    if q_start is None:
        print(f"[J6] {label}: cannot read joints for home return")
        return False
    ...
    _hold_joint_controller(r)
    time.sleep(0.05)
    print(f"[J6] {label}: returning home from {err0:.1f}° max error")
    ...
    if err1 > 5.0:
        ...
        print(
            f"[J6] WARNING: {label} failed to reach home "
    ...
    set_vec(r, GOAL_JOINTS, q_home_rad)
    print(f"[J6] {label}: home reached (err {err1:.2f}°)")
    return True
...
def run_loop(
    ...
    *,
    ...
    def finish_lock(reason: str, status: str | None = None) -> None:
        ...
        print(
            f"[J6 lock] throw {locked_throw_id} {status}: {reason}; "
    ...
    def print_lock_wait(reason: str) -> None:
        ...
        if now - last_lock_reject_print_t >= 0.50:
            ...
            print(f"[J6 lock] waiting for safe prediction: {reason}")
    ...
    print(f"[J6] strike_plane_x={strike_plane_x_world:+.3f} m  "
          ...
          f"pre_settle={pre_swing_settle_s:.2f} s")
    print(f"[J6] workspace (arm frame):  r_xy ≤ {reach_m:.2f} m  "
          f"z ∈ [{z_min:+.2f}, {z_max:+.2f}] m")
    print(f"[J6] strike box (arm frame, unclipped): "
    ...
    if using_mock:
        ...
        print(f"[J6] MOCK intercept(s) world/arm if identity (m): {pts}")
    ...
    if lock_first_safe_prediction:
        print(
            "[J6 lock] enabled: first safe prediction per throw is frozen; "
            "later tracker jitter is ignored"
        )
    else:
        print("[J6 lock] disabled: tracking will chase each new prediction")
    print("[J6] tracking limiter is time-based; slow Python/IK loops still command requested deg/s")
    if tracking_accel_deg_s2 > 0.0:
        print(f"[J6] tracking acceleration cap: {tracking_accel_deg_s2:.0f}°/s² command ramp")
    if direct_tracking:
    ...
    while True:
        ...
        if tti > commit_tti or no_commit:
            ...
            if ik_good and joints_good and delta_good:
                if lock_first_safe_prediction and locked_intercept is None:
                    ...
                    print(
                        f"[J6 lock] throw {locked_throw_id} LOCKED: "
                    ...
                    if debug_joints:
                        print(f"[J6 lock]   q_cur_deg={_fmt_deg_vec(q_cur)}")
                        print(f"[J6 lock]   q_tgt_deg={_fmt_deg_vec(q_wu)}")
                ...
                if quiet_hold_active:
                    ...
                    if not quiet_hold_printed:
                        print(
                            f"[J6 quiet] settled within {quiet_settle_deadband_deg:.1f}° "
        ...
        if not (tti > commit_tti or no_commit):
            if locked_intercept is None:
                reason = (
                    f"COMMIT skipped: no locked throw at tti={tti:.3f}s; "
                    "ignoring late prediction instead of chasing without a lock"
                )
                print(f"[J6] {reason}")
                recover_home_blocking("late-unlocked→home", idle_after=True)
                time.sleep(max(0.0, dt - (time.perf_counter() - t0)))
                continue
            ...
            print(
                f"[J6 timing] impact_in≈{tti_at_swing_start:.3f}s at swing start; "
            ...
            if seg["goal_err_max_deg"] > 5.0:
                ...
                print(f"[J6] *** {reason} — recovering home, no follow-through ***")
            ...
            if not mini_swing:
                ...
                follow_seg = run_segment(
                    r, "strike→follow", q_post_strike, q_follow,
                ...
                if follow_seg["goal_err_max_deg"] > 5.0:
                    ...
                    print(f"[J6] *** {reason} — recovering home ***")
...
def main() -> None:
    ...
    try:
        ...
        if args.shutdown_hold_s > 0.0:
            ...
            print(
                f"\n[J6] stopped — refreshed current joint hold for "
                f"{args.shutdown_hold_s:.1f}s."
            )
        else:
            _hold_current_joints(r)
            print("\n[J6] stopped — holding current joint position in Redis.")
