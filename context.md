...
- **2026-06-08** (J9 velocity-violation fix + big-ball config) — Diagnosed root cause of FR3 `joint_velocity_violation` reflex aborts: cubic Hermite from home (q4≈-115°) to q_strike (q4≈-150°) with positive endpoint velocity causes a mid-swing overshoot to q4≈-159°, where the FR3 position-dependent hard limit is only ~33 deg/s, far below the ~96 deg/s peak. Three changes applied to `stepj9_impact_velocity_planner.py`:
  1. **FR3 dynamic scaling gate** (`fr3_max_hard_ratio` param, default 0.90): after computing `_trajectory_diagnostics`, if the Hermite's worst FR3 hard-zone ratio exceeds `fr3_max_hard_ratio`, scale down `qdot_strike` proportionally. Recomputes q_stop, achieved_v_W, Tmin, and reruns diagnostics. Logged with `[J9 FR3] scale …` line.
  2. **FR3 ratio always shown** in main-loop `[J9]` print line (`fr3=NNN%@qN ABORT-RISK`), not just in verbose mode.
  3. **Velocity carry-in** (`v0` param in `_run_impact_strike`): reads actual joint velocities at commit time (`SENSOR_JOINT_VELS`) and passes them as the Hermite start velocity. Avoids a discontinuous velocity command when the arm is already moving toward q_strike in AIM state.
  - **New YAML:** `configs/j9_strike_fast_bigball.yaml` — for large foam ball (rigid body ID 1): `strike_plane_x=-0.35`, `fr3_max_hard_ratio=0.88`, `tracking_step_deg=2.5`, `commit_tti=1.00`, `strike_arm_z_min_m=0.20`.
  - **Mobile base position for pickleball sessions:** x=-1.0, y=0.0, yaw=0°. Set via `python sports_bot/scripts/send_base_goal.py --robot-rigid-body-id 11 --x -1.0 --y 0.0 --yaw-deg 0`.
  - **Throw from further back** improves tracking: more OptiTrack frames → valid intercept at larger TTI → more pre-positioning time → smaller Hermite travel → lower FR3 ratio.
...
- **2026-05-31** — J6 reactive intercept bring-up complete. `picklebot.xml` joint_controller gains finalized: `kp="200 200 200 200 400 600 400"` / `kv="20 20 20 20 28 28 28"` (velocity saturation + `velocityLimit`, no OTG). Three code fixes landed in `stepj6_reactive_intercept.py`: (1) commit path keep-tracking when arm is en route to wind-up (`tti>0`); (2) `strike→home` starts from actual sensor joints (not planned `q_strike`); (3) pre-swing active settle holds `last_wu_q` goal for 200 ms so all joints converge before wu→strike fires. Ball rigid body streaming ID confirmed = **1**. Live OptiTrack cart tracking verified: SETTLED at 1.1° residual. `frames.py` `compute_T_W_A_from_markers`: auto-assigns left/right marker by world-Y (lower-Y = left/-Y), eliminating Motive ID-swap flip bug; valid for ≤±90° cart rotation from forward-facing. `picklebot_clean.xml` (kp=100 uniform) archived for reference — insufficient kp to execute strike swing.
